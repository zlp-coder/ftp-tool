# coding=utf-8
import wx
from FrmDownTaskEdit import *
import wx.dataview as dvlib

class FrmDownTask(wx.Dialog):
    ...
    def OnDoubleClick(self, event):
        if self.dv.GetSelection().IsOk():
            wedit = FrmDownTaskEdit(self.pw, seq=self.idList[self.dv.GetSelectedRow()])
            wedit.ShowModal()
            self.LoadData()
    ...
    def OnlbDeleteClicked(self):
        if self.dv.SelectedRow >= 0:
            ...
            if lr == wx.ID_YES:
                self.pw.cfgManager.delDownTaskList(self.idList[self.dv.GetSelectedRow()])
                self.LoadData()
